...
template<class DataType>
DataType SUList<DataType>::operator[](const int x){
  ListNode* cursor=head;
  for(int i=0;i<x;i++){
    cursor=cursor->next;
  }
...
//Sorts the given array if it has the > operator
template<class DataType>
void SUList<DataType>::sort(){
  for(ListNode* cursor=head;cursor->next;cursor=cursor->next){
    for(ListNode* cursor=head;cursor->next;cursor=cursor->next){
